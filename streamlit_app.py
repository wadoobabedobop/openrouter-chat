...
ROUTER_MODEL_ID = "mistralai/mistral-7b-instruct:free" # CHANGED TO A MORE ROBUST ROUTER
# Other good free options if Mistral also struggles:
# ROUTER_MODEL_ID = "nousresearch/nous-hermes-2-mistral-7b-dpo:free"
# ROUTER_MODEL_ID = "nousresearch/nous-hermes-2-mixtral-8x7b-dpo:free" # Slower but very capable
...
ROUTER_MODEL_GUIDANCE = {
    ...
    "B": "(Model B: Solid Mid-Tier [Cost Rank 3/6]) Use for general chat, moderate reasoning, summarization, standard tasks IF 'F'/'D' are *clearly insufficient*. Also use for sensitive topics if 'A'/'E' are not available/needed. Good balance. Cheaper than A, E, C.",
    ...
    "D": "(Model D: Factual & Technical [Cost Rank 2/6]) Use for factual Q&A, code tasks, data extraction, straightforward logic IF 'F' is *too basic*. ***DO NOT USE for sensitive topics.*** Prefer over 'B'/'A'/'E'/'C' for these tasks if cost allows. Slow.",
    "E": "(Model E: Novel & Creative, High Cost [Cost Rank 5/6]) Use for highly *original/unique* creative content, brainstorming fresh angles, or a distinct non-corporate style, ONLY IF 'A'/'B' *lack the required creative spark* AND the high cost is justified. Also suitable for sensitive topics needing high nuance. Cheaper than C.",
...
# --- Main App Logic (Executes only if API key setup is NOT required) ---
    ...
    for msg_idx, msg in enumerate(chat_history): # Use enumerate for potential keys
        ...
        with st.chat_message(role, avatar=avatar_char):
             # Corrected: Removed the 'key' argument which caused the TypeError
        ...
        with st.spinner("Selecting best model..."): # More specific spinner text
            ...
            for k_map in cost_order_check:
                 if k_map in MODEL_MAP: # Only check models defined in our map
                     ...
                     logging.info(f"Model {k_map} ({MODEL_MAP.get(k_map,'?') }): Available = {available}")
                     if available:
